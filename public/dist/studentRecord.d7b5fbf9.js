let e;const t=document.querySelector(".drop-zone"),r=t.querySelector(".file-input"),a=document.querySelector(".form-text"),s=document.querySelector(".fa-cloud-arrow-up"),n=document.querySelector(".ongoing"),o=document.querySelector(".file-name"),l=document.querySelector(".size"),d=document.querySelector(".fa-spinner"),c=document.querySelector(".fa-square-check"),i=document.querySelector(".drag-drop-container"),u=document.querySelector(".student-record-container"),p=document.querySelectorAll(".page"),m=document.querySelector(".next"),v=document.querySelector(".prev"),f=document.querySelector(".main"),L=document.querySelector(".filter-container"),y=document.querySelector(".delete-btn");t.addEventListener("click",()=>{r.click()}),r.onchange=({target:e})=>{E(e.files[0])},t.addEventListener("dragover",e=>{e.preventDefault(),s.classList.add("uploading"),a.textContent="Drop the file"}),t.addEventListener("dragleave",()=>{s.classList.remove("uploading"),a.textContent="Browse File to Upload"}),t.addEventListener("drop",e=>{e.preventDefault(),s.classList.remove("uploading"),a.textContent="Browse File to Upload";let t=e.dataTransfer.files;t.length>0&&E(t[0])});const E=e=>{e&&(n.classList.add("active"),o.textContent=`${e.name} \u{2022} Uploading`,d.classList.remove("remove")),S(e)},S=async e=>{let t=new FormData;t.append("file",e);try{let r=await fetch("/app/api/upload",{method:"POST",body:t});if(r.ok)await r.json(),o.textContent=`${e.name} \u{2022} Uploaded`,l.textContent=`${Math.round(e.size/1024)} KB`,c.classList.remove("remove"),d.classList.add("remove"),setTimeout(()=>{i.classList.add("remove"),f.classList.remove("blurred"),C()},3e3);else throw Error("There was an error uploading the file.")}catch(t){o.textContent=`${e.name} \u{2022} Error`,console.error(t)}};let $="",g="",h="";L.addEventListener("submit",e=>{e.preventDefault();let t=Object.fromEntries(new FormData(L).entries());$=t.studentNumber?`studentNumber=${t.studentNumber}&`:"",g=t.yearLevel?`yearLevel=${t.yearLevel}&`:"",h=t.course?`course=${t.course}&`:"",b($,g,h)}),y.addEventListener("click",async e=>{e.preventDefault();try{let e=await fetch("/app/api/delete-record");if(!e.ok)throw Error("Failed to delete record");await e.json(),setTimeout(()=>{w=1,x=3,k=1,f.classList.add("blurred"),i.classList.remove("remove")},2e3)}catch(e){console.error(e)}});const q=e=>`
